...
def build_generators(t_ind):
    if t_ind == 0:
        ...
        generator = generators.VarGatedDelayedEstimationTask(max_iter=25001, batch_size=50, n_loc=2, n_in=50, n_out=1,
                                                             stim_dur=25, max_delay=100, resp_dur=25, kappa=2.0,
                                                             spon_rate=0.1, tr_cond='all_gains')
        test_generator = generators.VarGatedDelayedEstimationTask(max_iter=2501, batch_size=50, n_loc=2, n_in=50,
                                                                  n_out=1,
                                                                  stim_dur=25, max_delay=100, resp_dur=25, kappa=2.0,
                                                                  spon_rate=0.1, tr_cond='all_gains')
    elif t_ind == 6:
        generator = generators.VarHarvey2012(max_iter=25001, batch_size=50, n_in=50, n_out=1, stim_dur=25,
                                             max_delay=100,
                                             resp_dur=25, sigtc=15.0, stim_rate=1.0, spon_rate=0.1)
        test_generator = generators.VarHarvey2012(max_iter=2501, batch_size=50, n_in=50, n_out=1, stim_dur=25,
                                                  max_delay=100,
                                                  resp_dur=25, sigtc=15.0, stim_rate=1.0, spon_rate=0.1)
    elif t_ind == 8:
        generator = generators.VarComparisonTask(max_iter=25001, batch_size=50, n_loc=1, n_in=50, n_out=1, stim_dur=25,
                                                 max_delay=100, resp_dur=25, sig_tc=10.0, spon_rate=0.1,
                                                 tr_cond='all_gains')
        test_generator = generators.VarComparisonTask(max_iter=2501, batch_size=50, n_loc=1, n_in=50, n_out=1,
                                                      stim_dur=25,
                                                      max_delay=100, resp_dur=25, sig_tc=10.0, spon_rate=0.1,
                                                      tr_cond='all_gains')
    else:
...
def build_performance(s_vec, opt_vec, net_vec, t_ind):
    if t_ind == 0 or t_ind == 1 or t_ind == 4:
        ...
        performance = np.nanmean(
            opt_vec * np.log(opt_vec / net_vec) + (1.0 - opt_vec) * np.log((1.0 - opt_vec) / (1.0 - net_vec))) \
