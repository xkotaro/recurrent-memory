...
from dynamic_input_utils import build_generators, build_model, build_loss, build_performance
import scipy.io as sio

parser = argparse.ArgumentParser(description='Recurrent Memory Experiment (Dynamic Input Condition)')
...
rec_layer_fn = theano.function([input_var], lasagne.layers.get_output(l_rec, get_details=True),
...
ex_hid_vec = np.reshape(ex_hid_vec,
                        (-1, generator.stim_dur + generator.delay_dur + generator.resp_dur, ExptDict["n_hid"]))

ex_inp_vec = np.asarray(ex_inp_vec)
ex_inp_vec = np.reshape(ex_inp_vec, (
-1, generator.stim_dur + generator.delay_dur + generator.resp_dur, (ExptDict["task"]["n_loc"] + 1) * generator.n_in))

# SAVE TRAINED MODEL
sio.savemat('dynamic_sigma%f_lambda%f_rho%f_model%i_task%i.mat' % (offdiag_val, diag_val, wdecay_coeff, m_ind, t_ind),
