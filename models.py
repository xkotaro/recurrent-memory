...
def LeInitRecurrent(input_var, mask_var=None, batch_size=1, n_in=100, n_out=1,
    ...
    l_hid_hid = DenseLayer(lasagne.layers.InputLayer((None, n_hid)), n_hid,
                           ...
                           nonlinearity=lasagne.nonlinearities.linear)
    l_rec = lasagne.layers.CustomRecurrentLayer(l_in, l_in_hid, l_hid_hid, nonlinearity=lasagne.nonlinearities.softplus,
                                                mask_input=l_mask, grad_clipping=100)
...
def ResidualRecurrent(input_var, mask_var=None, batch_size=1, n_in=100, n_out=1, n_hid=200, leak_inp=1.0, leak_hid=1.0,
                      out_nlin=lasagne.nonlinearities.linear):
    # Input Layer
    l_in = InputLayer((batch_size, None, n_in), input_var=input_var)
    if mask_var == None:
    ...
    l_in_hid = DenseLayer(lasagne.layers.InputLayer((None, n_in)), n_hid, W=lasagne.init.GlorotNormal(0.1), b=None,
                          nonlinearity=lasagne.nonlinearities.linear)
    l_hid_hid = DenseLayer(lasagne.layers.InputLayer((None, n_hid)), n_hid, W=lasagne.init.GlorotNormal(0.1),
                           nonlinearity=lasagne.nonlinearities.linear)
    l_rec = ResidualCustomRecurrentLayer(l_in, l_in_hid, l_hid_hid, nonlinearity=lasagne.nonlinearities.rectify,
                                         mask_input=l_mask, grad_clipping=100, leak_inp=1.0, leak_hid=1.0)

    # Output Layer
    l_shp = ReshapeLayer(l_rec, (-1, n_hid))
    l_dense = DenseLayer(l_shp, num_units=n_out, W=lasagne.init.GlorotNormal(0.1), nonlinearity=out_nlin)
    # To reshape back to our original shape, we can use the symbolic shape variables we retrieved above.
    l_out = ReshapeLayer(l_dense, (batch_size, seqlen, n_out))
